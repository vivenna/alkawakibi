# -*- coding: utf-8 -*-
"""JavaScript der Website. Gemeinsamer Teil plus Zusätze je Entwurf."""

KONFIGURATION = """/* Einstellungen der Website.
 *
...
HAUPT = r"""/* Alkawakibi e. V. – Verhalten der Website.
 *
...
EINBLENDEN = r"""
  ...
  function einblendenEinrichten() {
    var elemente = alle('.abschnitt, .karte, .nummer-punkt, .person');
    if (!elemente.length || ruhig || !('IntersectionObserver' in window)) { return; }
    elemente.forEach(function (element) { element.classList.add('faehrt-ein'); });
    ...
    elemente.forEach(function (element) { beobachter.observe(element); });
  }
"""


def haupt_js(zusatz='', zusatz_start=''):
    return HAUPT.replace('__ZUSATZ__', zusatz).replace('__ZUSATZ_START__', zusatz_start)
