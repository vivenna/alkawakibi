...
/* Lange deutsche Komposita („Datenschutzerklärung“) sind in großen
   Überschriftengraden breiter als ein Telefonbildschirm. Sie werden getrennt
   statt über den Rand geschoben. */
...
/* Schlussformel der Satzung: die Unterschriftslinie des Originals */
.unterschrift { border-top: 1px solid; padding-top: .8rem; margin-top: 2.5rem; max-width: 34rem; }
...
.konto-daten { display: grid; grid-template-columns: max-content minmax(0, 1fr); gap: .4rem 1.2rem; margin: 0; }
...
/* Auf schmalen Bildschirmen reicht die Breite nicht für Beschriftung und
   Wert nebeneinander – sonst wird die IBAN rechts abgeschnitten. Dort
   stehen die Angaben untereinander und die IBAN darf an ihren Leerstellen
   umbrechen (also zwischen den Vierergruppen, nicht mitten in einer Zahl). */
@media (max-width: 36rem) {
  .konto-daten { grid-template-columns: minmax(0, 1fr); gap: 0; }
  .konto-daten dt { margin-top: .7rem; }
  .konto-daten dt:first-child { margin-top: 0; }
  .iban { white-space: normal; }
}
...
/* Der Schalter muss über dem geöffneten Menü liegen. Schiebt ein Entwurf das
   Menü als Tafel über den Bildschirm, verdeckt diese sonst den Schalter und
   das Menü lässt sich nicht mehr schließen. */
.menue-schalter {
  display: none; border: 0; background: none; align-items: center; gap: .5rem;
  position: relative; z-index: 2;
}
@media (max-width: 899px) {
  .hauptnavigation { z-index: 1; }
}
